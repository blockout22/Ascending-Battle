	...
	public static Mesh load(String objFile) {
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (line.startsWith("f ")) {
					for (int i = 1; i < line.split(" ").length; i++) {
						...
						normalIndex.add(Integer.valueOf(line.split(" ")[i].split("/")[2]) - 1);
					}
				}
			}

			float[] finalTexCoords = new float[vertices.size() * 3];
			...
			for (int i = 0; i < indices.size(); i++) {
				...
				int normalsPos = normalIndex.get(i);
			...
			Mesh mesh = new Mesh(Mesh.MESH_3D);
